...
int escolheFechamento(){
	...
	clearDisplay();
	lcdWritePos("fechamento.", 0, 0);
	lcdWritePos("pressione 1 a 3", 0, 1);
	
	while(sair == 0){
		leitura = le_teclado();
		if((leitura_ant == semTeclaPressionada) && (leitura != semTeclaPressionada)){
			if(fechamentoSelecionado != 0 && leitura == teclaConfirma){
				sair = 1;
			}
			else{
				switch(leitura){
					case '1':
						...
						lcdWritePos(descricao[0], 0, 0);
						lcdWritePos("favor confirmar", 0, 1);
						fechamentoSelecionado = 1;
					break;
					case '2':
						clearDisplay();
						lcdWritePos(descricao[1], 0, 0);
						lcdWritePos("favor confirmar", 0, 1);
						fechamentoSelecionado = 2;
					break;
					case '3':
						clearDisplay();
						lcdWritePos(descricao[2], 0, 0);
						lcdWritePos("favor confirmar", 0, 1);
						fechamentoSelecionado = 3;
					break;
			   	case '0':
					case '4':
					case '5':
					...
					case 'E':
						clearDisplay();
						lcdWritePos("3 opcoes.", 0, 0);
						lcdWritePos("pressione 1 a 3", 0, 1);
						fechamentoSelecionado = 0;
					break;
				}
			}
	  }
